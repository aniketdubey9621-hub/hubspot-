...
app.get('/dev/token-meta', async (_request, reply) => {
...
app.post('/dev/expire-token', async (_request, reply) => {
  if (process.env.DEV_ROUTES_ENABLED !== 'true') {
    reply.status(404);
    return { error: { code: 'not_found', message: 'Not found' } };
  }
...
async function start() {
  ...
  if (SERVE_STATIC && fs.existsSync(path.join(CLIENT_DIST, 'index.html'))) {
